            ...
            if complete == 1:
                ...
                for key, name in response.headers:
                    if key == 'etag':
                        etag = name[1:]
                        etag = etag[:-1]
                self.complete_cloud_to_device_message(etag)
        
        return response
    
    # Needs more work
    def complete_cloud_to_device_message(self, messageLockId):
        request = HTTPRequest()
        request.method = 'DELETE'
        request.host = self._get_host()
        request.path = '/devices/' + self.device_id + '/messages/devicebound/' + messageLockId + '?api-version=' + API_VERSION
        request.path, request.query = self._httpclient._update_request_uri_query(request)
        request.headers = self._update_iot_hub_header(request)
